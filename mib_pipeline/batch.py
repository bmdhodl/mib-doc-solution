...
class BatchRunner:
    ...
    def run(self, input_dir: Path, output_path: Path) -> BatchRunReport:
        return self.run_paths(discover_case_pdfs(input_dir), output_path)
    ...
    def run_paths(
        ...
        """Process an explicit, already ordered set of PDF paths."""
        ...
        if self._row_finalizer is not None:
            ...
            for pdf_path, result in zip(pdf_paths, results, strict=True):
                ...
                try:
                    ...
                    # The finalizer is an optional score lift. A valid base
                    # prediction is safer than turning a layout-only failure
                    # into a missing-case penalty.
                    finalized.append(result)
