...
contains one stronger, exact-case biometric value.  A second field-only
tie-breaker may reuse an already-triggered Rapid pass after visible repairs to
resolve a multi-way, one-digit sponsor disagreement without another OCR pass.
"""
...
from .models import PredictionRow, SPONSOR_ID_PATTERN
from .resolution import FieldState, ResolvedCase, ResolvedField
from .visible_candidate_complement import apply_visible_candidate_complement
...
COMPLETE_VISIBLE_CLEAN_REVIEW_APPROVAL_CONFIDENCE = 0.98
...
SEMANTIC_DENIAL_RULE_IDS = (
    ...
    "semantic_visible_transit",
    "semantic_dual_ocr_wolf_non_diplomatic",
)
_COMPLETE_VISIBLE_CLEAN_POLICY_FIELDS = (
    "home_world",
    "visa_class",
    "sponsor_id",
    "arrival_date",
...
_APPROVABLE_VISIBLE_VISAS = frozenset({"DIP-1", "XW-1", "XW-2", "MED-3"})
...
class RapidOutputRecoveryProcessor:
    ...
    def __init__(
        ...
        *,
        ...
        output_recalibrator: Any | None = None,
        row_finalizer: Callable[[Path, PredictionRow], PredictionRow] | None = None,
        post_field_recoverer: Any | None = None,
    ) -> None:
        ...
        self._output_recalibrator = output_recalibrator
        self._row_finalizer = row_finalizer
        self._post_field_recoverer = post_field_recoverer
    ...
    @staticmethod
    def _has_conflicting_authoritative_rapid_denial(
        *,
        case_id: str,
        primary_resolved: ResolvedCase,
        rapid_resolved: ResolvedCase,
        ...
        """Veto a semantic denial only for visible APPROVED/REVIEW authority."""
        ...
        return any(
            isinstance(candidate, CandidateEvidence)
            and candidate.field_name == "adjudication"
            and candidate.value in {"APPROVED", "NEEDS_REVIEW"}
            and candidate.evidence_type in AUTHORITATIVE_RAPID_TYPES
            and candidate.legible
            and not candidate.superseded
            and candidate.ocr_confidence >= AUTHORITATIVE_MINIMUM_CONFIDENCE
    ...
    @classmethod
    def _exact_case_visible_values(
        cls,
        *,
        case_id: str,
        field_name: str,
        ...
        """Return live visible values without consulting serialized fallbacks."""
    ...
    def _observed_non_none_risk(
        ...
        *,
        ...
        """Veto default-``none`` rows when either OCR saw a positive risk."""

        values = cls._exact_case_visible_values(
            case_id=case_id,
            field_name=RAPID_RISK_FIELD,
            candidates=tuple(primary_candidates) + tuple(rapid_candidates),
        )
        return any(bool(cls._parse_risk_flags(value)) for value in values)
    ...
    def _semantic_denial_rules(
        ...
        *,
        ...
        if (
            ...
            or cls._has_conflicting_authoritative_rapid_denial(
                case_id=case_id,
        ...
        no_positive_risk = bool(
            str(payload.get(RAPID_RISK_FIELD, "")).strip().casefold() == "none"
            and not cls._observed_non_none_risk(
                case_id=case_id,
                primary_candidates=primary_candidates,
                rapid_candidates=rapid_candidates,
            )
        )
        visible_transit = bool(
            payload.get("visa_class") == "TRANSIT-7"
            and (
                primary_values.get("visa_class") == "TRANSIT-7"
                or rapid_values.get("visa_class") == "TRANSIT-7"
            )
        )
        if visible_transit and no_positive_risk:
            matches.append(SEMANTIC_DENIAL_RULE_IDS[4])

        primary_worlds = cls._exact_case_visible_values(
            case_id=case_id,
            field_name="home_world",
            candidates=primary_candidates,
        )
        rapid_worlds = cls._exact_case_visible_values(
            case_id=case_id,
            field_name="home_world",
            candidates=rapid_candidates,
        )
        visible_visas = cls._exact_case_visible_values(
            case_id=case_id,
            field_name="visa_class",
            candidates=tuple(primary_candidates) + tuple(rapid_candidates),
        )
        dual_ocr_wolf = bool(
            payload.get("home_world")
            in SEMANTIC_POLICY_RULES.non_diplomatic_embargoed_worlds
            and payload.get("home_world") in primary_worlds
            and payload.get("home_world") in rapid_worlds
            and payload.get("visa_class") not in {None, "DIP-1"}
            and payload.get("visa_class") in visible_visas
        )
        if dual_ocr_wolf and no_positive_risk:
    ...
    @classmethod
    def _complete_visible_clean_review_approval(
        cls,
        *,
        final_row: PredictionRow,
        primary_outcome: AdjudicationOutcome,
        ...
        """Promote a fully visible clean policy state, never a row fallback."""
        ...
        if (
            ...
            or primary_outcome.trace.denial_reasons
            or cls._primary_authoritative_decision(primary_outcome)
            or (
                rapid_resolved is not None
                and cls._has_authoritative_rapid_decision(
            ...
            or final_row.visa_class not in _APPROVABLE_VISIBLE_VISAS
            or final_row.home_world in SEMANTIC_POLICY_RULES.embargoed_worlds
            or (
                final_row.home_world
                in SEMANTIC_POLICY_RULES.non_diplomatic_embargoed_worlds
                and final_row.visa_class != "DIP-1"
            )
            or (
                final_row.sponsor_id in SEMANTIC_POLICY_RULES.barred_sponsors
                and final_row.visa_class != "DIP-1"
            )
            or " ".join(final_row.risk_flags.strip().split()).casefold()
            != "none"
        ...
        visible_values = {
            field_name: cls._visible_resolved_value(
                case_id=primary_resolved.case_id,
                resolved=primary_resolved,
                field_name=field_name,
                unsafe_pages=unsafe_pages,
            )
            for field_name in _COMPLETE_VISIBLE_CLEAN_POLICY_FIELDS
        }
        if any(
            visible_values[field_name] != getattr(final_row, field_name)
            for field_name in _COMPLETE_VISIBLE_CLEAN_POLICY_FIELDS
        ):
            return final_row
        if cls._observed_non_none_risk(
            case_id=primary_resolved.case_id,
            primary_candidates=primary_candidates,
            rapid_candidates=(),
        ):
        ...
        payload = final_row.to_dict()
        payload["adjudication"] = "APPROVED"
        payload["confidence"] = COMPLETE_VISIBLE_CLEAN_REVIEW_APPROVAL_CONFIDENCE
        return PredictionRow.from_mapping(
            payload,
            fallback_case_id=final_row.case_id,
        )
    ...
    def _apply_review_approval_heads(
        ...
        *,
        ...
        primary_candidates = tuple(primary_candidates)
        rapid_candidates = tuple(rapid_candidates)
        recovered = cls._complete_visible_clean_review_approval(
            final_row=final_row,
            primary_candidates=primary_candidates,
    ...
    @classmethod
    def _apply_reused_rapid_sponsor_tiebreak(
        cls,
        final_row: PredictionRow,
        rapid_candidates: Iterable[CandidateEvidence],
    ) -> PredictionRow:
        """Resolve a one-digit sponsor split using an already-run Rapid pass.

        This is a field-only, post-policy tie-break.  It rejects any set that
        corroborates the current value and accepts only the unique global
        highest-confidence value when it is exactly one digit from the current
        sponsor.  The value must be supported by either a second distinct Rapid
        reading or two independent evidence types.  It never starts OCR.
        """

        current = final_row.sponsor_id
        if (
            not SPONSOR_ID_PATTERN.fullmatch(current)
            or current == "SPN-0000"
        ):
            return final_row

        scores: dict[str, float] = {}
        evidence_types: dict[str, set[EvidenceType]] = {}
        for candidate in rapid_candidates:
            if (
                not cls._clean_multisource_candidate(candidate)
                or candidate.field_name != "sponsor_id"
                or candidate.case_id_hint not in {None, final_row.case_id}
                or candidate.value is None
                or not SPONSOR_ID_PATTERN.fullmatch(candidate.value)
                or candidate.value == "SPN-0000"
                or candidate.value in SEMANTIC_POLICY_RULES.barred_sponsors
            ):
                continue
            scores[candidate.value] = max(
                scores.get(candidate.value, 0.0),
                float(candidate.ocr_confidence),
            )
            evidence_types.setdefault(candidate.value, set()).add(
                candidate.evidence_type
            )

        if not scores or current in scores:
            return final_row
        ranked = sorted(
            scores.items(),
        ...
        selected = ranked[0][0]
        if (
            sum(left != right for left, right in zip(current[4:], selected[4:]))
            != 1
        ):
            return final_row
        if len(scores) < 2 and len(evidence_types[selected]) < 2:
            return final_row

        payload = final_row.to_dict()
        payload["sponsor_id"] = selected
        return PredictionRow.from_mapping(
            payload,
            fallback_case_id=final_row.case_id,
        )
    ...
    def _recover(
        ...
        *,
        ...
        unknown_fields: frozenset[str],
        recover_risk: bool,
    ) -> tuple[PredictionRow, tuple[CandidateEvidence, ...]]:
        ...
        return (
            self._apply_review_approval_heads(
                final_row=final_row,
                primary_candidates=primary_candidates,
                primary_outcome=primary_outcome,
                primary_resolved=primary_resolved,
    ...
    def process_case(self, pdf_path: Path) -> PredictionRow:
        ...
        if not unknown_fields and not recover_risk:
            ...
            try:
                final_row, rapid_candidates = self._recover(
                    rendered=rendered,
                    primary_row=primary_row,
                    primary_candidates=primary_candidates,
                    primary_resolved=primary_resolved,
        ...
        if self._output_recalibrator is not None:
            final_row = self._output_recalibrator.recalibrate(final_row)
        if self._row_finalizer is not None:
            try:
                final_row = self._row_finalizer(pdf_path, final_row)
            except Exception:
                # Preserve the prior BatchRunner contract: a scoring-only
                # finalizer failure must not turn a valid case into an omission.
                pass
        final_row = apply_visible_candidate_complement(
            final_row,
            primary_candidates,
        )
        if self._post_field_recoverer is not None:
            try:
                final_row = self._post_field_recoverer.recover(
                    pdf_path,
                    final_row,
                    primary_candidates,
                )
            except Exception:
                # This retry is output-only and may always abstain.
                pass
        # Reuse the already-triggered Rapid pass only after every decision and
        # confidence stage, so this tie-break is mechanically sponsor-only.
        return self._apply_reused_rapid_sponsor_tiebreak(
            final_row,
            rapid_candidates,
        )
