...
from mib_pipeline.rapid_recovery import (
    COMPLETE_VISIBLE_CLEAN_REVIEW_APPROVAL_CONFIDENCE,
    RapidOcrEngine,
...
def processor(
    ...
    *,
    ...
    rapid_error=None,
    primary_candidates=("primary",),
    output_recalibrator=None,
    row_finalizer=None,
    post_field_recoverer=None,
):
    ...
    recovery = RapidOutputRecoveryProcessor(
        ...
        output_recalibrator=output_recalibrator,
        row_finalizer=row_finalizer,
        post_field_recoverer=post_field_recoverer,
    )
...
class RapidOutputRecoveryTests(unittest.TestCase):
    def test_reused_rapid_sponsor_tiebreak_selects_unique_best_one_digit_value(self):
        candidates = (
            evidence("sponsor_id", "SPN-8960", confidence=0.98),
            evidence("sponsor_id", "SPN-6860", confidence=0.87),
        )
        before = row(
            sponsor_id="SPN-8860",
        ...
        after = RapidOutputRecoveryProcessor._apply_reused_rapid_sponsor_tiebreak(
            before,
            candidates,
        )

        expected = before.to_dict()
        expected["sponsor_id"] = "SPN-8960"
        self.assertEqual(after.to_dict(), expected)

    def test_reused_rapid_sponsor_tiebreak_accepts_two_evidence_types_for_one_value(
        self,
    ):
        before = row(sponsor_id="SPN-4768")
        candidates = (
            evidence(
                "sponsor_id",
                "SPN-4766",
        ...
        self.assertEqual(after.sponsor_id, "SPN-4766")

    def test_reused_rapid_sponsor_tiebreak_abstains_on_global_confidence_tie(self):
        before = row(sponsor_id="SPN-8860")
        tied = (
            evidence("sponsor_id", "SPN-8960", confidence=0.91),
            evidence("sponsor_id", "SPN-8866", confidence=0.91),
        )

        self.assertEqual(
            RapidOutputRecoveryProcessor._apply_reused_rapid_sponsor_tiebreak(
                before,
                tied,
            ),
            before,
    ...
    def test_reused_rapid_sponsor_tiebreak_abstains_on_unsafe_evidence(self):
        before = row(sponsor_id="SPN-8860")
        unsafe = (
            evidence(
                "sponsor_id",
    ...
    def test_reused_rapid_sponsor_tiebreak_requires_multiway_or_two_source_support(
        self,
    ):
        before = row(sponsor_id="SPN-8860")
        single = (
            evidence("sponsor_id", "SPN-8960", confidence=0.99),
        )
    ...
    def test_reused_rapid_sponsor_tiebreak_abstains_when_current_value_is_present(
        self,
    ):
        before = row(sponsor_id="SPN-8860")
        corroborated_current = (
            evidence("sponsor_id", "SPN-8860", confidence=0.95),
            evidence("sponsor_id", "SPN-8960", confidence=0.99),
        )
    ...
    def test_reused_rapid_sponsor_tiebreak_abstains_for_higher_distant_value(self):
        before = row(sponsor_id="SPN-4768")
        candidates = (
            evidence("sponsor_id", "SPN-4786", confidence=0.99),
            evidence("sponsor_id", "SPN-4766", confidence=0.97),
        )
    ...
    def test_reused_rapid_sponsor_tiebreak_preserves_decision_and_confidence(self):
        before = row(
            sponsor_id="SPN-8860",
            adjudication="DENIED",
            confidence=0.73,
        ...
        self.assertEqual(after.sponsor_id, "SPN-8960")
        self.assertEqual(after.adjudication, before.adjudication)
        self.assertEqual(after.confidence, before.confidence)

    def test_process_reuses_single_triggered_rapid_pass_after_visible_repairs(self):
        rapid_candidates = (
            evidence("sponsor_id", "SPN-8960", confidence=0.98),
            evidence("sponsor_id", "SPN-6860", confidence=0.87),
        )
        primary = resolved_case(unknown={"sponsor_id"})
        rapid = resolved_case(
            values={"sponsor_id": "SPN-8960"},
            considered={"sponsor_id": rapid_candidates},
        )
        primary_row = row(
            sponsor_id="SPN-0000",
            adjudication="DENIED",
        ...
        recovery, _renderer, _linker, resolver, _adjudicator, factory = processor(
            primary,
            rapid,
        ...
        def visible_repair(current, _path):
            payload = current.to_dict()
            payload["sponsor_id"] = "SPN-8860"
            return PredictionRow.from_mapping(payload)

        with patch(
            "mib_pipeline.rapid_recovery.apply_visible_field_repairs",
            side_effect=visible_repair,
        ):
            result = recovery.process_case(Path(CASE_ID + ".pdf"))

        self.assertEqual(result.sponsor_id, "SPN-8960")
    ...
    def test_process_does_not_create_rapid_pass_for_sponsor_tiebreak(self):
        rapid_candidates = (
            evidence("sponsor_id", "SPN-2263", confidence=0.99),
        )
        primary = resolved_case()
        primary_row = row(sponsor_id="SPN-2283")
        recovery, _renderer, _linker, resolver, _adjudicator, factory = processor(
            primary,
            resolved_case(),
            primary_outcome=outcome(primary_row),
        ...
        self.assertEqual(result.sponsor_id, "SPN-2283")
        self.assertEqual(factory.calls, 0)
        self.assertEqual(resolver.calls, 1)

    def test_applies_visible_candidate_complement_after_primary_decision(self):
        calls = []

        class Recalibrator:
            def recalibrate(self, current):
                calls.append("output_recalibrator")
                payload = current.to_dict()
                payload["confidence"] = 0.52
                return PredictionRow.from_mapping(payload)

        def finalizer(_path, current):
            calls.append("visible_score_finalizer")
            self.assertEqual(current.declared_purpose, "reactor maintenance")
            self.assertEqual(current.confidence, 0.52)
            payload = current.to_dict()
        ...
        recovery, _renderer, _linker, resolver, adjudicator, factory = processor(
            ...
            resolved_case(),
            primary_outcome=outcome(primary_row),
            primary_candidates=(purpose,),
            output_recalibrator=Recalibrator(),
            row_finalizer=finalizer,
        )

        result = recovery.process_case(Path(CASE_ID + ".pdf"))

        expected = primary_row.to_dict()
        expected["declared_purpose"] = "field repair"
        expected["confidence"] = 0.61
        self.assertEqual(result.to_dict(), expected)
        self.assertEqual(
            calls,
            ["output_recalibrator", "visible_score_finalizer"],
        )
        self.assertEqual(resolver.calls, 1)
        self.assertEqual(adjudicator.calls, 1)
        self.assertEqual(factory.calls, 0)

    def test_applies_post_field_recovery_after_all_scoring(self):
        calls = []
        ...
        class Recoverer:
            def recover(self, _path, current, candidates):
                calls.append("post_field_recoverer")
                assert tuple(candidates) == ("primary",)
                payload = current.to_dict()
                payload["risk_flags"] = "biohazard_red"
                return PredictionRow.from_mapping(payload)
        ...
        recovery, _renderer, _linker, resolver, adjudicator, factory = processor(
            ...
            output_recalibrator=Recalibrator(),
            row_finalizer=finalizer,
            post_field_recoverer=Recoverer(),
        )

        result = recovery.process_case(Path(CASE_ID + ".pdf"))

        expected = primary_row.to_dict()
        expected["risk_flags"] = "biohazard_red"
        expected["confidence"] = 0.61
        self.assertEqual(result.to_dict(), expected)
        self.assertEqual(
            calls,
            [
                "output_recalibrator",
                "visible_score_finalizer",
                "post_field_recoverer",
            ],
        )
        self.assertEqual(resolver.calls, 1)
        self.assertEqual(adjudicator.calls, 1)
    ...
    def test_authority_vetoes_semantic_denial_and_visible_transit_triggers_it(self):
        anchor = evidence("risk_flags", None, applicant=None)
...
class GenericPolicySeamTests(unittest.TestCase):
    @staticmethod
    def complete_visible_candidates(values=None):
        values = {**BASE_VALUES, **(values or {})}
        return tuple(
            evidence(
                field_name,
    ...
    def test_complete_visible_clean_review_approves_and_authority_vetoes(self):
        candidates = self.complete_visible_candidates()
        considered = {
            candidate.field_name: (candidate,) for candidate in candidates
        }
        ...
        approved = (
            RapidOutputRecoveryProcessor._complete_visible_clean_review_approval(
                final_row=row(),
                primary_outcome=clean_outcome,
                primary_candidates=candidates,
        ...
        rapid_review = evidence(
            "adjudication",
            "NEEDS_REVIEW",
            evidence_type=EvidenceType.SIGNED_MANUAL_NOTE,
            confidence=0.97,
        )
        rapid_authority_vetoed = (
            RapidOutputRecoveryProcessor._complete_visible_clean_review_approval(
                final_row=row(),
                primary_outcome=clean_outcome,
                primary_candidates=candidates,
                primary_resolved=primary,
                rapid_candidates=(rapid_review,),
                rapid_resolved=resolved_case(
                    considered={"adjudication": (rapid_review,)}
                ),
            )
        )

        self.assertEqual(approved.adjudication, "APPROVED")
        self.assertEqual(
            approved.confidence,
            COMPLETE_VISIBLE_CLEAN_REVIEW_APPROVAL_CONFIDENCE,
        )
        self.assertEqual(vetoed.adjudication, "NEEDS_REVIEW")
        self.assertEqual(
            rapid_authority_vetoed.adjudication,
            "NEEDS_REVIEW",
        )

    def test_complete_visible_clean_review_rejects_unsupported_waiver(self):
        values = {"fee_status": "waived", "visa_class": "XW-2"}
        candidates = self.complete_visible_candidates(values)
        considered = {
            candidate.field_name: (candidate,) for candidate in candidates
        }
        ...
        self.assertEqual(unsupported, prediction)

    def test_complete_visible_clean_review_allows_diplomatic_waiver(self):
        values = {"fee_status": "waived", "visa_class": "DIP-1"}
        candidates = self.complete_visible_candidates(values)
        considered = {
            candidate.field_name: (candidate,) for candidate in candidates
        }
        ...
        approved = (
            RapidOutputRecoveryProcessor._complete_visible_clean_review_approval(
                ...
                primary_outcome=outcome(
                    prediction,
                    review_reasons=("risk_flags_unknown",),
                    approval_facts=(
                        "application_date_current_or_exempt",
                        "valid_fee_waiver",
                    ),
    ...
    def test_visible_transit_denial_vetoes_observed_rescinded_risk(self):
        transit = evidence(
            "visa_class",
            "TRANSIT-7",
            evidence_type=EvidenceType.SPONSOR_ATTESTATION,
        ...
        primary = resolved_case(
            values={"visa_class": "TRANSIT-7"},
            considered={"visa_class": (transit,)},
        )
        rapid = resolved_case()
        base = {
            **row(visa_class="TRANSIT-7").to_dict(),
            "risk_flags": "none",
        }

        matched = RapidOutputRecoveryProcessor._semantic_denial_rules(
            case_id=CASE_ID,
            payload=base,
            primary_candidates=(transit,),
            rapid_candidates=(),
            primary_resolved=primary,
            rapid_resolved=rapid,
            primary_outcome=outcome(row(visa_class="TRANSIT-7")),
        ...
        vetoed = RapidOutputRecoveryProcessor._semantic_denial_rules(
            case_id=CASE_ID,
            payload=base,
            primary_candidates=(transit, rescinded),
            rapid_candidates=(),
            primary_resolved=primary,
            rapid_resolved=rapid,
            primary_outcome=outcome(row(visa_class="TRANSIT-7")),
        ...
        self.assertIn("semantic_visible_transit", matched)
        self.assertNotIn("semantic_visible_transit", vetoed)

    def test_wolf_denial_requires_two_ocr_worlds_and_one_visible_visa(self):
        primary_world = evidence("home_world", "Wolf-1061c")
        rapid_world = evidence("home_world", "Wolf-1061c")
        rapid_visa = evidence("visa_class", "MED-3")
        primary = resolved_case(values={"home_world": "Wolf-1061c"})
        rapid = resolved_case(
            values={"home_world": "Wolf-1061c", "visa_class": "MED-3"}
        )
        payload = row(
            home_world="Wolf-1061c",
            visa_class="MED-3",
        ...
        matched = RapidOutputRecoveryProcessor._semantic_denial_rules(
            case_id=CASE_ID,
            payload=payload,
            primary_candidates=(primary_world,),
            rapid_candidates=(rapid_world, rapid_visa),
            primary_resolved=primary,
            rapid_resolved=rapid,
            primary_outcome=outcome(row(home_world="Wolf-1061c")),
            unknown_fields=frozenset(),
            recover_risk=False,
        )
        one_engine = RapidOutputRecoveryProcessor._semantic_denial_rules(
            case_id=CASE_ID,
            payload=payload,
            primary_candidates=(primary_world,),
            rapid_candidates=(rapid_visa,),
            primary_resolved=primary,
            rapid_resolved=rapid,
            primary_outcome=outcome(row(home_world="Wolf-1061c")),
            unknown_fields=frozenset(),
        ...
        no_visa = RapidOutputRecoveryProcessor._semantic_denial_rules(
            case_id=CASE_ID,
            payload=payload,
            primary_candidates=(primary_world,),
            rapid_candidates=(rapid_world,),
            primary_resolved=primary,
            rapid_resolved=rapid,
            primary_outcome=outcome(row(home_world="Wolf-1061c")),
            unknown_fields=frozenset(),
        ...
        self.assertIn("semantic_dual_ocr_wolf_non_diplomatic", matched)
        self.assertNotIn(
            "semantic_dual_ocr_wolf_non_diplomatic",
            one_engine,
        )
        self.assertNotIn(
            "semantic_dual_ocr_wolf_non_diplomatic",
